...
from map_and_refine.refine import TextRefiner
...
for file in files:
    with open(file, encoding='utf-8') as f:
        ...
        # Get output from full text refine
        summarizer = TextRefiner(llm=llm, model_name=model_name)
        summary, total_tokens_used = summarizer.refine(full_text)
        ...
        save_results(scores, Path('results/refine/results.csv'))

        # Also save the summary just in case
        basename, _ = os.path.splitext(os.path.basename(file))
        with open(Path(f'results/refine/summary_{basename}.txt'), 'w') as out:
            out.write(summary)
